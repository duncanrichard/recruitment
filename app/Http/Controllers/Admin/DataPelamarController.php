...
class DataPelamarController extends Controller
{
    ...
    public function kirimPesanSkrining(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'selected_ids.*' => ['required', 'uuid', Rule::exists('data_riwayat_diri', 'id')],
            'perusahaan_dilamar' => ['nullable', 'uuid', Rule::exists('data_perusahaan', 'id')],
        ], [
        ...
        if ($pelamars->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'Kandidat yang dipilih tidak ditemukan atau tidak sesuai dengan filter tanggal/perusahaan.',
                'total_pelamar' => 0,
                'total_data' => 0,
                'total_dikirim' => 0,
                'total_dilewati' => 0,
