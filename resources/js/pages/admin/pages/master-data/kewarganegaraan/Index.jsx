...
export default function KewarganegaraanPage({ actionSignals }) {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("/admin/master-data/kewarganegaraan/list", {
                headers: {
            ...
            if (result.success) {
                ...
                alert(result.message || "Gagal mengambil data kewarganegaraan.");
            }
        } catch (error) {
            console.error("Gagal mengambil data kewarganegaraan:", error);
            alert("Terjadi kesalahan saat mengambil data kewarganegaraan.");
        } finally {
    ...
    useEffect(() => {
        if (actionSignals?.masterKewarganegaraan > 0) {
            resetForm();
            setModalOpen(true);
        }
    }, [actionSignals?.masterKewarganegaraan]);
    ...
    const filteredData = useMemo(() => {
        ...
        if (!keyword) {
            return dataKewarganegaraan;
        }

        return dataKewarganegaraan.filter((item) => {
            const kewarganegaraan = String(item.kewarganegaraan || "").toLowerCase();

            return kewarganegaraan.includes(keyword);
        });
    }, [dataKewarganegaraan, search]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editId
                ? `/admin/master-data/kewarganegaraan/${editId}`
                : "/admin/master-data/kewarganegaraan";
            ...
            alert(result.message || "Data kewarganegaraan berhasil disimpan.");
            closeModal();
            fetchData();
        } catch (error) {
            console.error("Gagal menyimpan data kewarganegaraan:", error);
            alert("Terjadi kesalahan saat menyimpan data kewarganegaraan.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmDelete = confirm(
            "Yakin ingin menghapus data kewarganegaraan ini?"
        );

        if (!confirmDelete) return;

        try {
            const response = await fetch(`/admin/master-data/kewarganegaraan/${id}`, {
                method: "DELETE",
                headers: {
                    Accept: "application/json",
                    "X-CSRF-TOKEN": getCsrfToken(),
            ...
            if (result.success) {
                alert(result.message || "Data kewarganegaraan berhasil dihapus.");
                fetchData();
            } else {
                alert(result.message || "Data gagal dihapus.");
            }
        } catch (error) {
            console.error("Gagal menghapus data kewarganegaraan:", error);
            alert("Terjadi kesalahan saat menghapus data kewarganegaraan.");
        }
    };
                        ...
                        <thead>
                            <tr className="bg-slate-50/80">
                                ...
                                <SortableTableHead
                                    ...
                                    sortKey="kewarganegaraan"
                                    onSort={handleSort}
                                    icon={sortIcon("kewarganegaraan")}
                                />
                                    ...
                                    <td colSpan="3" className="px-6 py-16">
                                        <div className="mx-auto max-w-sm text-center">
                                            ...
                                            <h3 className="mt-4 text-lg font-black text-slate-900">
                                                Data kewarganegaraan tidak ditemukan
                                            </h3>

                                            <p className="mt-2 text-sm font-medium text-slate-500">
                                                Belum ada data kewarganegaraan atau kata kunci pencarian tidak cocok.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <p className="text-sm font-bold text-slate-500">
                        ...
                        {search && (
                            <span>
                                {" "}
                                filtered from {dataKewarganegaraan.length} total entries
                            </span>
                        )}
                    </p>
                            ...
                            <div className="flex items-center justify-between gap-4 px-6 py-5">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm font-medium text-slate-500">
                                        Lengkapi nama kewarganegaraan yang akan digunakan sebagai referensi.
                                    </p>
                                </div>
