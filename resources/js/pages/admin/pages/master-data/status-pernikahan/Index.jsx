...
export default function StatusPernikahanPage({ actionSignals }) {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(
                "/admin/master-data/status-pernikahan/list",
                {
                    headers: {
            ...
            if (result.success) {
                ...
                alert(result.message || "Gagal mengambil data status pernikahan.");
            }
        } catch (error) {
            console.error("Gagal mengambil data status pernikahan:", error);
            alert("Terjadi kesalahan saat mengambil data status pernikahan.");
        } finally {
    ...
    useEffect(() => {
        if (actionSignals?.masterStatusPernikahan > 0) {
            resetForm();
            setModalOpen(true);
        }
    }, [actionSignals?.masterStatusPernikahan]);
    ...
    const filteredData = useMemo(() => {
        ...
        if (!keyword) {
            return dataStatusPernikahan;
        }

        return dataStatusPernikahan.filter((item) => {
            const value = String(item.status_pernikahan || "").toLowerCase();

            return value.includes(keyword);
        });
    }, [dataStatusPernikahan, search]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editId
                ? `/admin/master-data/status-pernikahan/${editId}`
                : "/admin/master-data/status-pernikahan";
            ...
            alert(result.message || "Data status pernikahan berhasil disimpan.");
            closeModal();
            fetchData();
        } catch (error) {
            console.error("Gagal menyimpan data status pernikahan:", error);
            alert("Terjadi kesalahan saat menyimpan data status pernikahan.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmDelete = confirm(
            "Yakin ingin menghapus data status pernikahan ini?"
        );
        ...
        try {
            const response = await fetch(
                `/admin/master-data/status-pernikahan/${id}`,
                {
                    method: "DELETE",
                    headers: {
                        Accept: "application/json",
            ...
            if (result.success) {
                alert(result.message || "Data status pernikahan berhasil dihapus.");
                fetchData();
            } else {
                alert(result.message || "Data gagal dihapus.");
            }
        } catch (error) {
            console.error("Gagal menghapus data status pernikahan:", error);
            alert("Terjadi kesalahan saat menghapus data status pernikahan.");
        }
    };
                        ...
                        <thead>
                            <tr className="bg-slate-50/80">
                                ...
                                <SortableTableHead
                                    ...
                                    sortKey="status_pernikahan"
                                    onSort={handleSort}
                                    icon={sortIcon("status_pernikahan")}
                                />
                                    ...
                                    <td colSpan="3" className="px-6 py-16">
                                        <div className="mx-auto max-w-sm text-center">
                                            ...
                                            <h3 className="mt-4 text-lg font-black text-slate-900">
                                                Data status pernikahan tidak ditemukan
                                            </h3>

                                            <p className="mt-2 text-sm font-medium text-slate-500">
                                                Belum ada data status pernikahan atau kata kunci pencarian tidak cocok.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <p className="text-sm font-bold text-slate-500">
                        ...
                        {search && (
                            <span>
                                {" "}
                                filtered from {dataStatusPernikahan.length} total entries
                            </span>
                        )}
                    </p>
                            ...
                            <div className="flex items-center justify-between gap-4 px-6 py-5">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm font-medium text-slate-500">
                                        Lengkapi status pernikahan yang akan digunakan sebagai referensi.
                                    </p>
                                </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="space-y-5 px-6 py-6">
                                <Input
                                    ...
                                    placeholder="Contoh: Belum Menikah, Menikah, Cerai"
                                    required
                                />
                            </div>
