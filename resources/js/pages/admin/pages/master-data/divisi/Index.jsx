...
export default function DivisiPage({ actionSignals }) {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("/admin/master-data/divisi/list", {
                headers: {
            ...
            if (result.success) {
                setDataDivisi(result.data || []);
            } else {
                alert(result.message || "Gagal mengambil data divisi.");
            }
        } catch (error) {
            console.error("Gagal mengambil data divisi:", error);
            alert("Terjadi kesalahan saat mengambil data divisi.");
        } finally {
    ...
    useEffect(() => {
        if (actionSignals?.masterDivisi > 0) {
            resetForm();
            setModalOpen(true);
        }
    }, [actionSignals?.masterDivisi]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editId
                ? `/admin/master-data/divisi/${editId}`
                : "/admin/master-data/divisi";
            ...
            alert(result.message || "Data divisi berhasil disimpan.");
            closeModal();
            fetchData();
        } catch (error) {
            console.error("Gagal menyimpan data divisi:", error);
            alert("Terjadi kesalahan saat menyimpan data divisi.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmDelete = confirm("Yakin ingin menghapus data divisi ini?");

        if (!confirmDelete) return;

        try {
            const response = await fetch(`/admin/master-data/divisi/${id}`, {
                method: "DELETE",
                headers: {
                    Accept: "application/json",
                    "X-CSRF-TOKEN": getCsrfToken(),
            ...
            if (result.success) {
                alert(result.message || "Data divisi berhasil dihapus.");
                fetchData();
            } else {
                alert(result.message || "Data gagal dihapus.");
            }
        } catch (error) {
            console.error("Gagal menghapus data divisi:", error);
            alert("Terjadi kesalahan saat menghapus data divisi.");
        }
    };
                                    ...
                                    <td colSpan="3" className="px-6 py-16">
                                        <div className="mx-auto max-w-sm text-center">
                                            ...
                                            <p className="mt-2 text-sm font-medium text-slate-500">
                                                Belum ada data divisi atau kata kunci pencarian tidak cocok.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <p className="text-sm font-bold text-slate-500">
                        ...
                        {search && (
                            <span>
                                {" "}
                                filtered from {dataDivisi.length} total entries
                            </span>
                        )}
                    </p>
                            ...
                            <div className="flex items-center justify-between gap-4 px-6 py-5">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm font-medium text-slate-500">
                                        Lengkapi nama divisi yang akan digunakan sebagai referensi.
                                    </p>
                                </div>
