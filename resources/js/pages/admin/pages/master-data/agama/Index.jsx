...
export default function AgamaPage({ actionSignals }) {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("/admin/master-data/agama/list", {
                headers: {
            ...
            if (result.success) {
                ...
                alert(result.message || "Gagal mengambil data agama.");
            }
        } catch (error) {
            console.error("Gagal mengambil data agama:", error);
            alert("Terjadi kesalahan saat mengambil data agama.");
        } finally {
    ...
    useEffect(() => {
        if (actionSignals?.masterAgama > 0) {
            resetForm();
            setModalOpen(true);
        }
    }, [actionSignals?.masterAgama]);
    ...
    const filteredData = useMemo(() => {
        ...
        return dataAgama.filter((item) => {
            const agama = String(item.agama || "").toLowerCase();

            return agama.includes(keyword);
        });
    }, [dataAgama, search]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editId
                ? `/admin/master-data/agama/${editId}`
                : "/admin/master-data/agama";
            ...
            alert(result.message || "Data agama berhasil disimpan.");
            closeModal();
            fetchData();
        } catch (error) {
            console.error("Gagal menyimpan data agama:", error);
            alert("Terjadi kesalahan saat menyimpan data agama.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmDelete = confirm("Yakin ingin menghapus data agama ini?");

        if (!confirmDelete) return;

        try {
            const response = await fetch(`/admin/master-data/agama/${id}`, {
                method: "DELETE",
                headers: {
                    Accept: "application/json",
                    "X-CSRF-TOKEN": getCsrfToken(),
            ...
            if (result.success) {
                alert(result.message || "Data agama berhasil dihapus.");
                fetchData();
            } else {
            ...
            alert("Terjadi kesalahan saat menghapus data agama.");
        }
    };
                                    ...
                                    <td colSpan="3" className="px-6 py-16">
                                        <div className="mx-auto max-w-sm text-center">
                                            ...
                                            <p className="mt-2 text-sm font-medium text-slate-500">
                                                Belum ada data agama atau kata kunci pencarian tidak cocok.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <p className="text-sm font-bold text-slate-500">
                        ...
                        {search && (
                            <span>
                                {" "}
                                filtered from {dataAgama.length} total entries
                            </span>
                        )}
                    </p>
                            ...
                            <div className="flex items-center justify-between gap-4 px-6 py-5">
                                <div>
                                    ...
                                    <h2 className="mt-2 text-2xl font-black text-slate-950">
                                        {editId ? "Edit Agama" : "Tambah Agama"}
                                    </h2>

                                    <p className="mt-1 text-sm font-medium text-slate-500">
                                        Lengkapi nama agama yang akan digunakan sebagai referensi.
                                    </p>
                                </div>
