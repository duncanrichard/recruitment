...
export default function JabatanPage({ actionSignals }) {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("/admin/master-data/jabatan/list", {
                headers: {
            ...
            if (result.success) {
                ...
                alert(result.message || "Gagal mengambil data jabatan.");
            }
        } catch (error) {
            console.error("Gagal mengambil data jabatan:", error);
            alert("Terjadi kesalahan saat mengambil data jabatan.");
        } finally {
    ...
    useEffect(() => {
        if (actionSignals?.masterJabatan > 0) {
            resetForm();
            setModalOpen(true);
        }
    }, [actionSignals?.masterJabatan]);
    ...
    const filteredData = useMemo(() => {
        ...
        return dataJabatan.filter((item) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editId
                ? `/admin/master-data/jabatan/${editId}`
                : "/admin/master-data/jabatan";
            ...
            alert(result.message || "Data jabatan berhasil disimpan.");
            closeModal();
            fetchData();
        } catch (error) {
            console.error("Gagal menyimpan data jabatan:", error);
            alert("Terjadi kesalahan saat menyimpan data jabatan.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmDelete = confirm("Yakin ingin menghapus data jabatan ini?");

        if (!confirmDelete) return;

        try {
            const response = await fetch(`/admin/master-data/jabatan/${id}`, {
                method: "DELETE",
                headers: {
                    Accept: "application/json",
                    "X-CSRF-TOKEN": getCsrfToken(),
            ...
            if (result.success) {
                alert(result.message || "Data jabatan berhasil dihapus.");
                fetchData();
            } else {
            ...
            alert("Terjadi kesalahan saat menghapus data jabatan.");
        }
    };
                                    ...
                                    <td colSpan="3" className="px-6 py-16">
                                        <div className="mx-auto max-w-sm text-center">
                                            ...
                                            <p className="mt-2 text-sm font-medium text-slate-500">
                                                Belum ada data jabatan atau kata kunci pencarian tidak cocok.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <p className="text-sm font-bold text-slate-500">
                        ...
                        {search && (
                            <span>
                                {" "}
                                filtered from {dataJabatan.length} total entries
                            </span>
                        )}
                    </p>
                            ...
                            <div className="flex items-center justify-between gap-4 px-6 py-5">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm font-medium text-slate-500">
                                        Lengkapi nama jabatan yang akan digunakan sebagai referensi.
                                    </p>
                                </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="space-y-5 px-6 py-6">
                                <Input
                                    ...
                                    placeholder="Contoh: Direktur, Manager, Staff HRD"
                                    required
                                />
                            </div>
