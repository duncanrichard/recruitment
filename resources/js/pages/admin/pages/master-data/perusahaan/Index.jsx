...
export default function DataPerusahaanPage({ actionSignals }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const url = editId
                ? `/admin/master-data/perusahaan/${editId}`
                : "/admin/master-data/perusahaan";
            ...
            alert(result.message || "Data perusahaan berhasil disimpan.");
            closeModal();
            fetchData();
        } catch (error) {
            console.error("Gagal menyimpan data perusahaan:", error);
            alert("Terjadi kesalahan saat menyimpan data perusahaan.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        const confirmDelete = confirm("Yakin ingin menghapus data perusahaan ini?");

        if (!confirmDelete) return;

        try {
            const response = await fetch(`/admin/master-data/perusahaan/${id}`, {
                method: "DELETE",
                headers: {
                    Accept: "application/json",
                    "X-Requested-With": "XMLHttpRequest",
            ...
            if (result.success) {
                alert(result.message || "Data perusahaan berhasil dihapus.");
                fetchData();
            } else {
            ...
            alert("Terjadi kesalahan saat menghapus data perusahaan.");
        }
    };
                                    ...
                                    <td colSpan="6" className="px-6 py-16">
                                        <div className="mx-auto max-w-sm text-center">
                                            ...
                                            <p className="mt-2 text-sm font-medium text-slate-500">
                                                Belum ada data perusahaan atau kata kunci pencarian tidak cocok.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <p className="text-sm font-bold text-slate-500">
                        ...
                        {search && (
                            <span>
                                {" "}
                                filtered from {dataPerusahaan.length} total entries
                            </span>
                        )}
                    </p>
